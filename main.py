...
def main():
    ...
    # player2 = Player(GRID_SIZE - 2 , GRID_SIZE - 2, player_frames, explosion_range = 1, score = 0, heal = 2, bomb_num = 1)
    ...
    while running:
        ...
        if keyboard.is_pressed('shift'):
            ...
            if len(bombs_list) < player2.bomb_num and not any(bomb.tile_x == player2_tile_x and bomb.tile_y == player2_tile_y for bomb in bombs_list):
                # 调整爆炸范围到 3 格
                bombs_list.append(Bomb(player2_tile_x, player2_tile_y, player2.explosion_range, player2))
                player2.placed_bomb = 1

        if keyboard.is_pressed('/'):
            # +8是爲了補償player.cheak_collision的偏移
            player_tile_x = (player2.x + 8) // TILE_SIZE
            player_tile_y = (player2.y + 8) // TILE_SIZE
            if not any(plant.tile_x == player_tile_x and plant.tile_y == player_tile_y for plant in plant_list):
                plant_list.append(Plant(player_tile_x, player_tile_y))
                player2.placed_water = 1
        ...
        # if (player.heal <= 0 or player2.heal <= 0 or 
        #     game_map.grid[player.x // TILE_SIZE][player.y // TILE_SIZE] == TileType.UNBREAKABLE or 
        #     game_map.grid[player2.x // TILE_SIZE][player2.y // TILE_SIZE] == TileType.UNBREAKABLE):
